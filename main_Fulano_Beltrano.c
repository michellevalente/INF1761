...
int erode_cb(void)
{
	...
	if (tmp != NULL){
		imgErode(tmp);
		repaint_cb(gc.canvas);   /* repaint canvas */
		IupSetfAttribute(gc.msgbar, "TITLE", "Dilatation");
	}
    return IUP_DEFAULT;
}
...
int reverse_cb(void)
{
	...
	if (tmp != NULL){
		imgReverse(tmp);
		repaint_cb(gc.canvas);   /* repaint canvas */
		IupSetfAttribute(gc.msgbar, "TITLE", "Trocar preto com o branco");
	}
    return IUP_DEFAULT;
}
...
Ihandle* InitToolbar(void)
{
    ...
    /* Create four buttons */
	 ...
	 Ihandle* erode = IupSButton("img/erode.bmp", "Erodir", (Icallback)erode_cb);
	 Ihandle* dilate = IupSButton("img/dilate.bmp", "Dilatar", (Icallback)dilate_cb);
	 Ihandle* count = IupSButton("img/count.bmp", "Contar", (Icallback)count_cb);
	 Ihandle* save = IupSButton("img/file_save.bmp", "save image in a BMP file", (Icallback)save_cb);
	 Ihandle* reverse = IupSButton("img/pencil.bmp", "Troca preto e branco", (Icallback)reverse_cb);

     toolbar=IupHbox(load,binary,erode,dilate,count,reverse,save, NULL);
